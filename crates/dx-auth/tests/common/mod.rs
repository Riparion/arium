...
//! `crates/dx-auth/migrations/sqlite/`. That gives us real SQL semantics
...
pub async fn make_user(pool: &SqlitePool, email: &str, password: &str) -> i64 {
    let user_id = dx_auth::auth::create_password_user(pool, email, password)
        .await
...
/// `totp_rs` params as `dx_auth::auth::check_totp` so the codes line up.
...
pub mod test_provider {
    ...
    /// `dx_auth::oauth::upsert_oauth_user(...)` directly, so the only thing
